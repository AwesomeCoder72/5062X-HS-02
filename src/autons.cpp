...
void red_positive() {
  ...
  if (chassis.odom_x_direction_get() == false) {
    chassis.pid_turn_set(74, 70);
  } else {
    chassis.pid_turn_set(65, 70);
  }
  ...
  if (chassis.odom_x_direction_get() == false) {
    ...
    chassis.pid_drive_set(-10.1_in, 90, true);
  ...
  if (chassis.odom_x_direction_get() == false) {
    chassis.pid_drive_set(-38_in, 100, true);
...
void skillz() {
  ...
  chassis.pid_turn_set(194, TURN_SPEED);
  chassis.pid_wait();
